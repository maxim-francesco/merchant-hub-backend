...
const TENANT_ID = '66f3b724-4917-46ec-8932-48240844a0ea';

async function main() {
  // 1) Mock Stripe key so storefront checkout works WITHOUT real payments
  ...
  console.log('OK: mock Stripe key set on tenant settings');

  // 2) Categories (expectedAttributes ['unit'] so the admin product form surfaces the unit field)
  const categories = [
  ...
  for (const c of categories) {
    const cat = await prisma.category.upsert({
      where: { tenantId_slug: { tenantId: TENANT_ID, slug: c.slug } },
      update: { name: c.name, expectedAttributes: c.expectedAttributes },
      create: { tenantId: TENANT_ID, name: c.name, slug: c.slug, expectedAttributes: c.expectedAttributes },
    });
    catIds[c.slug] = cat.id;
    console.log('OK: category ' + c.name + ' -> ' + cat.id);
  }

  // 3) Products (no images; imageUrl null — client adds later). unit/description/featured live in attributes.
  const products = [
  ...
  for (const p of products) {
    const prod = await prisma.product.upsert({
      where: { tenantId_slug: { tenantId: TENANT_ID, slug: p.slug } },
      update: { name: p.name, price: p.price, stock: p.stock, categoryId: catIds[p.cat],
                attributes: { unit: p.unit, description: p.description, featured: p.featured, imageUrl: null } },
      create: { tenantId: TENANT_ID, categoryId: catIds[p.cat], name: p.name, slug: p.slug,
                price: p.price, stock: p.stock,
                attributes: { unit: p.unit, description: p.description, featured: p.featured, imageUrl: null } },
    });
    console.log('OK: product ' + p.name + ' -> ' + prod.id);
  }
