...
export async function processCheckout(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    const stripeSecretKey = settings.stripeSecretKey || settings.paymentGateways?.stripe?.secretKey;

    if (!stripeSecretKey) {
      res.status(400).json({
        status: 'error',
        message: 'Store payment gateway not configured.',
      });
      return;
    }
    ...
    const { createdOrder, lineItems } = await prisma.$transaction(async (tx) => {
      const productIds = items.map((item: CheckoutItemInput) => item.productId);
    ...
    if (stripeSecretKey.startsWith('sk_test_mock') || stripeSecretKey === 'sk_test_placeholder') {
      ...
      sessionUrl = `http://localhost:8080/store/${tenant.slug}/success?order_id=mock_cs_${createdOrder.id}`;
    } else {
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        success_url: `http://localhost:8080/store/${tenant.slug}/success?order_id={CHECKOUT_SESSION_ID}`,
        cancel_url: `http://localhost:8080/store/${tenant.slug}`,
      });
